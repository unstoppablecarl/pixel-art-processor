...
export class GridToTileSheetPaintBuffer {
  ...
  paintAlphaMask(color: Color32, brush: PaintAlphaMask, x: number, y: number): TileId[]
  paintAlphaMask(color: Color32, brush: PaintAlphaMask, startX: number, startY: number, endX: number, endY: number): TileId[]
  paintAlphaMask(
    color: Color32,
    ...
    forEachLinePoint(x0, y0, x1, y1, (px, py) => {
      ...
      this.eachTile(scratch, (tile, tileLeft, tileTop, bX, bY, bW_t, bH_t) => {
        ...
        for (let i = 0; i < bH_t; i++) {
          ...
          const dS = (canvasY - tileTop) * tileSize + (bX - tileLeft)
  ...
  paintBinaryMask(color: Color32, brush: PaintBinaryMask, x: number, y: number): TileId[]
  paintBinaryMask(color: Color32, brush: PaintBinaryMask, startX: number, startY: number, endX: number, endY: number): TileId[]
  paintBinaryMask(
    color: Color32,
  ...
  paintRect(color: Color32, brush: PaintRect, x: number, y: number): TileId[]
  paintRect(color: Color32, brush: PaintRect, startX: number, startY: number, endX: number, endY: number): TileId[]
  paintRect(
    color: Color32,
    brush: PaintRect,
    x0: number,
    ...
    const { w: bW, h: bH, centerOffsetX, centerOffsetY } = brush
  ...
  /**
   * Iterates over tiles that overlap `bounds`, yielding the tile's data buffer
   * alongside the absolute pixel rect within that tile.
   * Null grid cells (no WangTile set) are skipped silently.
   */
  private eachTile(
    bounds: Rect,
    callback: (
    ...
    for (let ty = tyMin; ty <= tyMax; ty++) {
      ...
      for (let tx = txMin; tx <= txMax; tx++) {
        ...
        callback(tile, tileLeft, tileTop, bX, bY, bW, bH)
      }
    }
  }
}