...
export function makeTileRenderer(
  ...
  let pixelData = new PixelData(new ImageData(state.scaledTileSize, state.scaledTileSize))

  function resize() {
    pixelCanvas.resize(state.scaledTileSize, state.scaledTileSize)
    pixelData.set(new ImageData(state.scaledTileSize, state.scaledTileSize))
    tileSync.reset()
    queueRender()
  }
